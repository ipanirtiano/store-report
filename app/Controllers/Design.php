...
class Design extends BaseController
{
    ...
    public function index($tgl)
    {
        ...
        $kode_transaksi = 'DSN-' . $generetKode1 . $generetKode2;
        ...
        if ($tanggal_form) {
            $data_store = $this->designModel->where('tanggal', $tanggal_form)->findAll();
        } else {
            $data_store =  $this->designModel->where('bulan', $bulan)->findAll();
        }
    ...
    public function d_excel($tgl)
    {
        ...
        return view('design/excel_design', $data);
    }
    ...
    public function edit_design($kode_transaksi)
    {
        // get data transaksi by kode
        $data_transaksi = $this->designModel->where('kode_transaksi', $kode_transaksi)->first();
        ...
        return view('design/update', $data);
    }
    ...
    public function proses_update($kode_transaksi)
    {
        ...
        if (!$this->validate([
            ...
            return redirect()->to(base_url('/admin/edit-design/' . $kode_transaksi))->withInput();
        } else {
            // update data
    ...
    public function hapus_design($kode_transaksi)
    {
        $tanggal = date('d-m-Y');
        $date_encode = base64_encode($tanggal);
        // get id 
